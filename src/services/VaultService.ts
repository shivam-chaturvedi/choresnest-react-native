...
export const VaultService = {
  ...
  ensureLocalUri: async (
    ...
    if (document.localUri) {
      ...
      try {
        ...
        if (exists) {
          ...
          const uri = document.localUri.startsWith('file://')
            ? document.localUri
            : `file://${normalized}`;
        ...
        console.log(
          `VaultService: localUri exists in DB but file is missing on disk: ${document.localUri}`,
        );
      } catch (e) {
    ...
    try {
      console.log(
        `VaultService: Downloading document ${document.id} from remote...`,
      );
      const localUri = await VaultStorageService.downloadToLocalCache(
        document.remotePath,
        document.id,
        onDownloadProgress,
      );
      ...
      console.log(
        `VaultService: Document ${document.id} cached locally at ${localUri}`,
      );
      return localUri;
    } catch (e) {
      console.error(
        `VaultService: Failed to download document ${document.id} from remote`,
        e,
      );
      return null;
    }
  ...
  addDocument: async (data: DocumentInput) => {
    ...
    if (!localUri) {
      console.error(
        'VaultService: localUri or filePath is required to add a document',
      );
      return null;
    }
    ...
    try {
      ...
      NotificationScheduler.syncDocumentReminders(createdDoc, {
  ...
  updateDocument: async (id: string, updates: DocumentInput) => {
    ...
    let shouldTrackExpiry = false;
    const expiryFields: Array<keyof DocumentInput> = [
      'expiryDate',
      'warrantyTillDate',
      'nextServiceDate',
    ];
    ...
    try {
      await getDatabase().write(async () => {
        ...
        if (doc.profileId !== profileId) {
          console.warn(
            'VaultService: refusing to update document that does not belong to active profile',
          );
          return;
        }

        const prevMeta = doc.meta || {};
        const expiryChanged = expiryFields.some(field => {
          const directValue = updates[field];
          const metaValue = (updates.meta as Record<string, any> | undefined)?.[
            field
          ];
          const candidate = directValue ?? metaValue;
          return (
            candidate !== undefined &&
            candidate !== null &&
            candidate !== '' &&
            candidate !== prevMeta[field as string]
          );
        });
        if (expiryChanged) {
        ...
        await doc.update(d => {
          ...
          if (updates.sharedWithIds !== undefined)
            d.sharedWithIds = updates.sharedWithIds;
  ...
  deleteDocument: async (id: string): Promise<DeleteDocumentResult> => {
    ...
    try {
      await getDatabase().write(async () => {
        ...
        if (doc.profileId !== profileId) {
          console.warn(
            'VaultService: refusing to delete document that does not belong to active profile',
          );
          return;
        }
        snapshot = {

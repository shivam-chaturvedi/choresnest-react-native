...
export const AppSettingsService = {
  ...
  hasAnyProfileCompletedOnboarding: async (): Promise<boolean> => {
    try {
      ...
      if (
        errorMessage.includes('underlyingAdapter') ||
        errorMessage.includes('database is being reset')
      ) {
        return false;
      }
      console.error(
        'AppSettingsService: Error checking global onboarding status:',
        error,
      );
      return false;
    }
  ...
  completeOnboarding: async (profileId?: string | null): Promise<void> => {
    ...
    if (!resolvedProfileId) {
      console.warn(
        'AppSettingsService: Cannot complete onboarding without profile_id',
      );
      return;
    }
    const now = Date.now();

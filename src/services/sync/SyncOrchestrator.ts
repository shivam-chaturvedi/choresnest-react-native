...
const logChangeSetSummary = (
  ...
  console.log(
    `🧾 ${table} fetched ${total} rows (${changeSet.created.length} created, ${changeSet.updated.length} updated, ${changeSet.deleted.length} deleted)`,
  );
};

const collectTableFetchResults = async (
  descriptors: TableFetchDescriptor[],
): Promise<Record<string, TableChangeSet>> => {
  const results = await Promise.allSettled(
    descriptors.map(desc => desc.fetcher()),
  );
...
const getLatestPulledTimestamp = (
  tableChangesMap: Record<string, TableChangeSet>,
): number =>
...
const syncPhases = Array.from(
  new Set(SYNC_TABLES.map(config => config.phase)),
).sort();
...
export class SyncOrchestrator {
  ...
  async run(readOnly: boolean): Promise<void> {
    if (this.userId === GUEST_PROFILE_ID) {
      console.log(
        'Guest profile detected in SyncOrchestrator - skipping synchronize()',
      );
      return;
    }
      ...
      pushChanges: readOnly
        ...
        : async ({ changes }) => {
            ...
            await this.pushToServer(
              changes as Record<string, TableChangeSet | undefined>,
            );
          },
  ...
  private async pullChanges({
    ...
    const lastPulled = lastPulledAt
      ? new Date(lastPulledAt).toISOString()
      : new Date(0).toISOString();
    console.log(
      `PULL_START${label ? ` (${label})` : ''} for user ${
        this.userId
      }; readOnly=${readOnly}; since=${lastPulled}`,
    );

    const buildDescriptor = (
      config: TableSyncConfig,
    ): TableFetchDescriptor | null => {
      if (config.localOnly) {
        return null;
      }
      return {
    ...
    for (const phase of syncPhases) {
      const tablesInPhase = SYNC_TABLES.filter(
        config => config.phase === phase && !config.localOnly,
      );
    ...
    Object.entries(tableChangesMap).forEach(([table, changeSet]) =>
      logChangeSetSummary(table, changeSet),
    );
    const lastPulledMs = lastPulledAt ? new Date(lastPulledAt).getTime() : 0;
    const aggregatedTimestamp = Math.max(
      lastPulledMs,
      getLatestPulledTimestamp(tableChangesMap),
    );
    return {
      changes: SYNC_TABLES.reduce<Record<string, TableChangeSet>>(
        (acc, config) => {
          acc[config.key] = tableChangesMap[config.key] ?? {
            created: [],
            updated: [],
            deleted: [],
          };
  ...
  private async pushToServer(
    changes: Record<string, TableChangeSet | undefined>,
  ): Promise<void> {
    ...
    try {
      const memberRecords = await getDatabase()
        .get('members')
      ...
      console.error(
        'SyncOrchestrator: Failed to fetch allowed profile IDs:',
        e,
      );
    }
    ...
    const phaseResults = [] as PromiseSettledResult<{
      success: boolean;
      errors: number;
      table?: string;
    }>[][];
    for (const phase of syncPhases) {
      ...
      const tasks = tablesInPhase.map(config => {
        ...
        }).then(result => ({ ...result, table: config.key }));
      });
      if (tasks.length > 0) {
    ...
    const totalErrors = pushResults.reduce((acc, result) => {
      if (result.status === 'fulfilled') {
        if (result.value.errors > 0 && result.value.table) {
          failedTables.push(`${result.value.table}(${result.value.errors})`);
        }
    ...
    if (totalErrors > 0) {
      const tableHint = failedTables.length
        ? ` Tables: ${failedTables.join(', ')}.`
        : '';
      throw new Error(
        `Sync push failed for ${this.userId}. Total errors: ${totalErrors}.${tableHint} See prior Failed to upsert/delete logs for PostgREST details.`,
      );
    }
  }
}

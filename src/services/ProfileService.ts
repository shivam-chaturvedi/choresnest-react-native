...
import {
  setActiveProfile,
  GUEST_PROFILE_ID as DATABASE_GUEST_PROFILE_ID,
} from '../database';
...
const tryReadCachedSupabaseSession = async (): Promise<string | null> => {
  try {
    ...
    const userId =
      parsed?.currentSession?.user?.id ?? parsed?.session?.user?.id;
...
export const ProfileService = {
  ...
  async getActiveProfileId(): Promise<string | null> {
    ...
    try {
      ...
      console.error(
        'ProfileService: Unexpected error resolving profile ID:',
        error,
      );
    }
  ...
  async resetCache() {
    console.log(
      'ProfileService: Resetting in-memory cache and AsyncStorage profile keys',
    );
    cachedProfileId = null;
    setActiveProfile(DATABASE_GUEST_PROFILE_ID);
    try {
      ...
      console.error(
        'ProfileService: Failed to clear AsyncStorage during reset',
        error,
      );
    } finally {
      NotificationCenter.setActiveProfileId(null);

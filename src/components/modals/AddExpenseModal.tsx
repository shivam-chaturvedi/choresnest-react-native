...
interface AddExpenseModalProps {
  ...
  onUpdate?: (id: string, expense: ExpenseData) => void;
  transactionToEdit?: {
    id: string;
    name: string;
    amount: number;
...
const resolveCategoryId = (rawCategory: string | undefined): string => {
...
export const AddExpenseModal: React.FC<AddExpenseModalProps> = ({
  ...
  const [entryStage, setEntryStage] = useState<'category' | 'entry'>(
    'category',
  );
  ...
  const selectedCategoryColorKey = CategoryColorService.normalizeCategoryKey(
    selectedCategory?.id || 'groceries',
  );
  const selectedCategoryColor =
    categoryColors[selectedCategoryColorKey] ?? CATEGORY_COLOR_FALLBACK;
  ...
  const willExceedBudget =
    type === 'expense' &&
    categoryBudget > 0 &&
    currentCategorySpending + newAmount > categoryBudget;
  const percentOfBudget =
    categoryBudget > 0
      ? ((currentCategorySpending + newAmount) / categoryBudget) * 100
      : 0;
  ...
  const handleSubmit = () => {
    ...
    if (isEditing && transactionToEdit && onUpdate) {
      onUpdate(transactionToEdit.id, payload);
      resetAllFields();
      onClose();
      return;
    }
  ...
  useEffect(() => {
    ...
    if (transactionToEdit) {
      ...
      setCategory(resolveCategoryId(transactionToEdit.category));
      setDate(transactionToEdit.date || getLocalYYYYMMDD(new Date()));
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      setEntryStage(
                        option.type === 'expense' ? 'category' : 'entry',
                      );
                    }}
                  >
            ...
            {type === 'expense' && entryStage === 'category' && (
              ...
              >
                ...
                >
                  {categories.map(cat => {
                    const normalizedKey =
                      CategoryColorService.normalizeCategoryKey(cat.id);
                    const catColor =
            ...
            {(type === 'income' || entryStage === 'entry') && (
              <>
                  ...
                  >
                    <View style={styles.alertHeader}>
                      ...
                      <View style={styles.alertTexts}>
                        ...
                        >
                          This will exceed your {selectedCategory?.name} budget
                          by ₹
                          {(
                ...
                {type === 'expense' && entryStage === 'entry' && (
                  <View
                    style={[styles.entryHeader, { borderColor: colors.border }]}
                  >
                ...
                <Button
                  ...
                  style={[
                    ...
                    {
                      backgroundColor:
                        type === 'expense' ? colors.primary : colors.success,
                    },
                  ]}
                >
                ...
                {type === 'expense' && recentEntries.length > 0 && (
                  <View
                    style={[styles.recentList, { borderColor: colors.border }]}
                  >
                    <Text
                      style={[
                        styles.sectionLabel,

...
export const NotificationPanel: React.FC<NotificationPanelProps> = ({
  ...
  useEffect(() => {
    let active = true;
    NotificationScheduler.ensureExactAlarm(false)
      .then((enabled) => {
        if (active) setExactAlarmEnabled(enabled);
      })
      .catch(() => {
        if (active) setExactAlarmEnabled(NotificationScheduler.isExactAlarmEnabled());
      });
    return () => {
      active = false;
    };
  ...
  const sections = useMemo(() => {
    ...
    if (groups.today.length > 0) list.push({ title: "TODAY", data: groups.today, id: "today" });
    if (groups.earlier.length > 0) list.push({ title: "EARLIER", data: groups.earlier, id: "earlier" });
    return list;
  }, [groups]);
    ...
    >
      ...
      <View style={styles.cardContent}>
        ...
        <View style={styles.textContainer}>
          ...
          <Text style={[styles.itemDetail, { color: colors.mutedForeground }]} numberOfLines={2}>
            {item.detail}
          </Text>

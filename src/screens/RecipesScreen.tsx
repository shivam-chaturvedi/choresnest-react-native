...
export const RecipesScreen: React.FC = () => {
  ...
  useEffect(() => {
    void trackScreen('RecipesScreen');
  }, []);
  ...
  const [recipeEditType, setRecipeEditType] = useState<
    'Text' | 'Image' | 'Link' | 'Audio' | undefined
  >(undefined);
  ...
  const locationAwareRecipes = useMemo(() => {
    return recipes.filter(
      recipe =>
        !recipe.countryCode || recipe.countryCode === currentCountry.code,
    );
    ...
    try {
      ...
      if (collection && collection.recipeIds) {
        ...
        if (recipesInCollection.length === 0) {
          showToast({
            title: 'Info',
            description: 'No valid recipes found in collection',
            type: 'default',
          });
          return;
        }
        ...
        showToast({
          ...
          description: `Adding items from ${collection.name}...`,
          type: 'default',
        });
        ...
        showToast({
          title: 'Success',
          description: `Added recipes from ${collection.name} to list`,
          type: 'success',
        });
      } else {
        showToast({
    ...
    >
      <View style={styles.recommendLeft}>
        ...
        <View>
          ...
          {recipe.time ? (
            <Text
              style={[styles.recommendMeta, { color: colors.mutedForeground }]}
            >
              {recipe.time}
            </Text>
          ) : null}
  ...
  const renderForYou = () => {
    if (recipes.length === 0) {
      return (
        ...
        >
          <Text
            style={[styles.emptyContentTitle, { color: colors.foreground }]}
          >
            Add a recipe to unlock this space
          </Text>
          <Text
            style={[
              styles.emptyContentSubtitle,
              { color: colors.mutedForeground },
            ]}
          >
            Your For You, Quick Meals, and Favorites are generated from recipes
            and bookmarks you create. Tap + to add your first one.
          </Text>
        </View>
      );
    }
      ...
      <>
        ...
        >
          ...
          <View style={styles.groceryNoteRow}>
            ...
            <Text
              style={[styles.groceryNote, { color: colors.mutedForeground }]}
            >
              4 ingredients available from your grocery list
            </Text>
          </View>
        </View>
        ...
        {activePreferences.length > 0 && (
          ...
          >
            ...
            {recommended.length > 0 ? (
              recommended.map(renderRecommendedItem)
            ) : (
              <Text style={{ color: colors.mutedForeground, padding: 8 }}>
                No recipes match these preferences.
              </Text>
            )}
          </View>
        )}
          ...
          >
            {quickMeals.map(recipe => (
              ...
              >
                ...
                <Text
                  style={[styles.quickMeta, { color: colors.mutedForeground }]}
                >
                  {recipe.time}
                </Text>
              </Pressable>
        ...
        >
          <View style={styles.planContent}>
            ...
            <Text style={[styles.planMeta, { color: colors.mutedForeground }]}>
              Auto-generate grocery lists from recipes
            </Text>
          </View>
          <Pressable
            style={[
              ...
              <View style={styles.recipeInfo}>
                <View style={styles.recipeHeader}>
                  ...
                  <Pressable onPress={() => toggleBookmark(recipe.id)}>
                    <AppIcon
                      ...
                      size={20}
                      color={
                        recipe.saved ? colors.primary : colors.mutedForeground
                      }
                      style={recipe.saved ? { opacity: 1 } : { opacity: 0.5 }}
                    />
                  </Pressable>
                ...
                <View style={styles.tagsRow}>
                  {recipe.tags.map(tag => (
                    ...
                    >
                      <Text
                        style={[styles.smallTagText, { color: colors.primary }]}
                      >
                        {tag}
                      </Text>
                    </View>
    ...
    <>
      {collections.length === 0 ? (
        ...
        >
          ...
          >
            Collections group the recipes you add. Tap “Create Collection” below
            to start curating your own sets.
          </Text>
        </View>
      ) : (
          ...
          {collections.map(collection => (
            ...
            >
              ...
              <Text style={styles.collectionTitle}>
                {collection.name.substring(collection.name.indexOf(' ') + 1)}
              </Text>
              ...
              <View style={styles.collectionFooter}>
                ...
                />
                <Pressable
                  style={[styles.addAllButton, { borderRadius: radius.sm }]}
                  onPress={() => handleAddCollectionToGrocery(collection.id)}
                >
                  <AppIcon
                    name="shoppingCart"
                    size={14}
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            showToast({
              title: 'Deleted',
              description: `${recipe.name} removed`,
              type: 'default',
            });
            setShowRecipeDetail(false);
    ...
    <>
      ...
      <ImageGalleryModal
        ...
        images={
          ...
          (selectedRecipe?.image ? [selectedRecipe.image] : [])
        } // Fallback to single image if array empty (though logic prioritizes array)

...
const HomeScreenContent: React.FC = () => {
  ...
  const getIconColor = (base: string) =>
    isMidnight ? colors.foreground : base;
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        if (error) {
          console.error(
            `[HomeScreen] Supabase events fetch failed for profile ${profileId}:`,
            error,
          );
          return;
        }
        ...
        console.log(
          `[HomeScreen] Supabase events for profile ${profileId}:`,
          data,
        );
      } catch (error) {
        if (!isMounted) {
        ...
        console.error(
          `[HomeScreen] Unexpected error fetching supabase events for profile ${profileId}:`,
          error,
        );
      }
    };
  ...
  const timeZone = useMemo(
    () => safeTimeZone(currentCountry?.timeZone),
  ...
  const parseEventDateTime = (
    event: any,
  ): { dateTime: Date | null; isAllDay: boolean } => {
    const isAllDay = !event?.time || event.time === 'All Day';
    const dateTime = parseDateTimeInZone(
      event?.date,
      currentCountry.timeZone,
      event?.time,
    );
  ...
  useEffect(() => {
    const previousRead = new Map(
      notificationAlerts.map(alert => [alert.id, alert.read]),
    );
    ...
    const highPriorityTasks = tasks.filter(
      (t: any) => t.priority === 'high' && t.status === 'pending',
    );
  ...
  const renderPairRow = (
    ...
    <View style={styles.detailPairRow}>
      <View
        style={[
          styles.detailRowHalf,
          {
            borderColor: colors.border,
            backgroundColor: colors.card + '05',
            borderRadius: radius.md,
          },
        ]}
      >
  ...
  const quickActions: {
  ...
  }[] = [
    {
      ...
      color: getIconColor(colors.info),
      bg: colors.info + '25',
    },
    {
      label: 'Finance',
      iconName: 'wallet',
      action: () => (navigation as any).navigate('Expenses'),
      color: accentColor,
    ...
    ...(ENABLE_RECIPE_AND_MEALS
      ? [
          {
            label: 'Recipe',
            iconName: 'utensils' as AppIconName,
            action: () =>
      ...
      : [
          {
            label: 'Vault',
            iconName: 'lock' as AppIconName,
            action: () =>
              (navigation as any).navigate('home', { screen: 'Vault' }),
            color: accentColor,
            bg: colors.muted,
          },
        ]),
  ...
  const pendingGroceries = (groceryList || []).filter(
  ...
  const documentsCount = useMemo(() => {
    ...
    const memberCount = Object.values(memberVaults || {}).reduce(
      (total: number, docs: any) => total + docs.length,
      0,
    );
  ...
  const mealSummary = MEAL_TYPES.map(mealType => {
    try {
      const plannedMeal = (todayMeals || []).find(
        meal => meal.mealType === mealType.key,
      );
  ...
  const glanceMetrics = [
    ...
    {
      label: 'Shopping',
      value: (groceryList || []).filter((item: any) => !item.completed).length,
    },
  ];
  ...
  return (
    <>
        ...
        >
          ...
          >
            ...
            <View style={styles.membersRow}>
              {(members || []).map((member: any) => {
                const profileColor =
                  PROFILE_COLORS.find((c: any) => c.value === member.color)
                    ?.hex || colors.primary;
                return (
                  <Pressable
                    ...
                    onPress={() => {
                      ...
                      try {
                        if (member && member.id) {
                          console.log(
                            'Calling setActiveMember with ID:',
                            member.id,
                          );
                  ...
                  >
                    ...
                    >
                      <MemberIcon
                        ...
                        size={28}
                        color={member?.isActive ? '#fff' : colors.foreground}
                      />
                      {member?.isActive && (
                        <View
                          style={[
              ...
              {glanceMetrics.map((metric, index) => (
                <View
                  ...
                  style={[
                    ...
                    index < glanceMetrics.length - 1 &&
                      styles.glanceStatSpacing,
                  ]}
                >
                  <Text
                    style={[
                      styles.glanceStatValue,
                      { color: colors.primaryForeground },
                    ]}
                  >
                    {metric.value}
                ...
                >
                  <View
                    style={[
                      ...
                      { backgroundColor: action.bg, borderRadius: radius.sm },
                    ]}
                  >
                    <AppIcon
              ...
              >
                {todaysScheduleItems.map(item => (
                  ...
                  >
                    ...
                    >
                      <AppIcon
                        name={item.type === 'task' ? 'checkSquare' : 'user'}
                        size={14}
                        color={colors.primaryForeground}
                      />
                    </View>
          ...
          <View style={{ marginBottom: 24 }}>
            ...
            {isGuest && (
                      ...
                      { text: 'Cancel', style: 'cancel' },
                      {
                        ...
                        onPress: async () => {
                          try {
                            ...
                            console.error(
                              'Error logging out from guest mode:',
                              error,
                            );
                          }
                        },
              ...
              >
                ...
                <View style={styles.alertText}>
                  ...
                  <Text
                    style={{ marginTop: 2, fontSize: 13, color: '#856404' }}
                  >
            ...
            {visibleAlerts.length > 3 && (
              <Pressable
                ...
                onPress={openNotificationsPanel}
              >
                <Text style={{ fontSize: 14, color: accentColor }}>
                  Show all alerts
                </Text>
              </Pressable>
            )}
          ...
          <View style={styles.statsGrid}>
            <Pressable
              ...
              onPress={() =>
                (navigation as any).navigate('MainTabs', { screen: 'lists' })
              }
            >
              <View
                style={[
              ...
              <View style={styles.detailModalHeader}>
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      {formatDateTimeValue(
                        selectedEvent.dateString || selectedEvent.date,
                        selectedEvent.time,
                      ) || 'No Info Available'}
                    </Text>
                  </View>
              ...
              >
                ...
                {renderPairRow(
                  ...
                  selectedEvent.isRecurring
                    ? selectedEvent.recurrenceRule || 'Recurring schedule'
                    : null,
                  'Reminder',
                  selectedEvent.reminderOffsetMinutes !== undefined &&
                    selectedEvent.reminderOffsetMinutes !== null
                    ? `${selectedEvent.reminderOffsetMinutes} minute${
                        selectedEvent.reminderOffsetMinutes === 1 ? '' : 's'
                      } before`
                    : null,
                )}
                {renderPairRow(
                  ...
                  (() => {
                    ...
                    const member = members.find(
                      (m: any) => m.id === selectedEvent.memberId,
                    );
      ...
      />
      <FamilyOnboarding
        open={showFamilyOnboarding}
        onClose={() => setShowFamilyOnboarding(false)}
      />
    </>
  );
};

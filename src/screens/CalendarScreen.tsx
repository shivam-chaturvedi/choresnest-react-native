...
type CalendarListEntry =
  ...
  | (Pick<Task, 'id' | 'icon' | 'date' | 'priority'> & {
      type: 'task';
      title: string;
      time: string;
      memberId?: string;
...
type TaskServiceEventRecord = ObservableValue<
  ReturnType<typeof TaskService.observeEvents>
>;
type TaskServiceTaskRecord = ObservableValue<
  ReturnType<typeof TaskService.observeTasks>
>;
...
const getNextRecurringOccurrence = (
  event: CalendarEvent,
  reference: Date,
  ...
  const recurrenceEnd = event.recurrenceEndDate
    ? parseDateTimeInZone(event.recurrenceEndDate, eventTimeZone)
    : null;
  ...
  for (let i = 0; i < limit; i += 1) {
    if (recurrenceEnd && isAfter(candidateDate, startOfDay(recurrenceEnd)))
      return null;
...
const getNextCandidateForEntry = (
  ...
  if (entry.type === 'event' && entry.isRecurring) {
    const recurring = getNextRecurringOccurrence(
      entry,
      reference,
      entryTimeZone,
    );
...
const DraggableEvent: React.FC<{
  ...
  const profileColor = PROFILE_COLORS.find(
    (c: any) => c.value === member?.color,
  );
  const bgColor = profileColor
    ? profileColor.hex + '40'
    : colors.primary + '40';
  const borderColor = profileColor ? profileColor.hex : colors.primary;
  const priorityMeta =
    event.type === 'task' ? getCalendarPriorityMeta(event.priority) : null;
  ...
  const onHandlerStateChangeBegan = (
    eventData: PanGestureHandlerStateChangeEvent,
  ) => {
    ...
    try {
      ...
      if (eventData.nativeEvent.state === State.END) {
        ...
        if (activeView === 'Week') {
          ...
          const colShift = Math.round(
            deltaX / (Dimensions.get('window').width / 3),
          );
          if (colShift !== 0) {
            const eventTimeZone = event.timeZone || timeZone;
            const currentZoned =
              parseDateTimeInZone(event.date, eventTimeZone, event.time) ||
              new Date();
            currentZoned.setDate(currentZoned.getDate() + colShift);
            newDate = safeFormatInTimeZone(
              currentZoned,
              eventTimeZone,
              'yyyy-MM-dd',
            );
        ...
        if (hours >= 0 && hours < 24) {
          ...
          if (durationMatch && endMatch) {
            ...
            if (durationMatch[3].toUpperCase() === 'PM' && startH !== 12)
              startH += 12;
            if (durationMatch[3].toUpperCase() === 'AM' && startH === 12)
              startH = 0;
            ...
            const eDisplayHours =
              eHours === 0
                ...
                ? eHours - 12
                : eHours;
            const formattedEndTime = `${eDisplayHours}:${eMinutes
              .toString()
              .padStart(2, '0')} ${ePeriod}`;
        ...
        >
            ...
            >
              <Text
                style={{
                  fontSize: 10,
                  fontWeight: '900',
                  color: event.type === 'task' ? '#ef4444' : '#8b5cf6',
                }}
              >
          ...
          {event.type === 'task' && priorityMeta && (
            ...
            >
              <Text
                style={[
                  styles.calendarPriorityBadgeText,
...
const CalendarScreenContent: React.FC = () => {
  ...
  const accentIconColor = (fallback: string) =>
    isMidnight ? colors.foreground : fallback;
  useEffect(() => {
  ...
  const observeEvents = useCallback(
    () => TaskService.observeEvents(profileId),
    [profileId],
  );
  ...
  const observeTasks = useCallback(
    () => TaskService.observeTasks(profileId),
    [profileId],
  );
  ...
  const events = useMemo<CalendarEventWithMeta[]>(() => {
    ...
    return rawEvents.reduce<CalendarEventWithMeta[]>((acc, record) => {
      ...
      const sanitizedEndTime = rawEndTime
        ? normalizeTimeString(rawEndTime)
        : undefined;
      const endTimeArg =
  ...
  const formatZoned = useCallback(
    (value: Date | undefined, pattern: string): string => {
      if (!value) return '';
  ...
  const scrollToCurrentTime = useCallback(
    (targetDate: Date | null = null, animated = true) => {
      ...
      if (activeView === 'Day') {
        ...
        shouldScroll = isWithinInterval(todayZoned, {
      ...
      const minute = parseInt(
        safeFormatInTimeZone(now, timeZone, 'm', '0'),
        10,
      );
  ...
  const filteredEvents = useMemo<CalendarEventWithMeta[]>(() => {
    ...
    return events.filter(
      (e: CalendarEventWithMeta) => e.memberId === filterMember,
    );
  }, [events, filterMember]);
  ...
  const currentTimeHours = parseInt(
    safeFormatInTimeZone(now, timeZone, 'H', '0'),
    10,
  );
  const currentTimeMinutes = parseInt(
    safeFormatInTimeZone(now, timeZone, 'm', '0'),
    10,
  );
  const currentTimeTop =
    currentTimeHours * HOUR_HEIGHT + currentTimeMinutes * (HOUR_HEIGHT / 60);
  ...
  const upcomingItems = useMemo<UpcomingEntry[]>(() => {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      dayEvents.forEach(item => {
        ...
        const itemTime =
          item.time && item.time !== 'All Day' ? item.time : '12:00 AM';
        ...
        if (
          !entries.find(
            e => e.id === item.id && isSameDay(e.nextDate, pseudoDate),
          )
        ) {
  ...
  const todayUpcomingItems = useMemo<UpcomingEntry[]>(() => {
    ...
    const targetDateStr = safeFormatInTimeZone(
      zonedNow,
      timeZone,
      'yyyy-MM-dd',
    );
    ...
    return upcomingItems.filter(
      item =>
        safeFormatInTimeZone(item.nextDate, timeZone, 'yyyy-MM-dd') ===
        targetDateStr,
    );
  ...
  // ---------------------------------------------------------
  const [groupedEventsForMonth, setGroupedEventsForMonth] = useState<
    Map<string, any[]>
  >(() => new Map());
  ...
  useEffect(() => {
    ...
    const computeHandle = InteractionManager.runAfterInteractions(() => {
      ...
      const todayKey =
        safeFormatInTimeZone(todayDate, timeZone, 'yyyy-MM-dd') || '';
      ...
      const otherDays = monthDays.filter(
        d => !todayInMonth || !isSameDay(d, todayInMonth),
      );
      let index = 0;
      ...
      >
        ...
        <View style={styles.grid}>
          {monthDays.map((day, i) => {
            ...
            const dateKey =
              safeFormatInTimeZone(day, timeZone, 'yyyy-MM-dd') || '';
              ...
              >
                ...
                <View style={styles.eventDotRx}>
                  {dayEvents.slice(0, 3).map((e, idx) => {
                    ...
                    const dotColor =
                      memberColorMap.get(e.memberId) || accentColor;
        ...
        >
          {(activeView === 'Day' || activeView === 'Week') && (
            <View style={styles.dayHeader}>
              <Text
                style={[styles.dayHeaderNumber, { color: colors.foreground }]}
              >
                {formatZoned(selectedDate, 'd')}
              </Text>
              <View>
                <Text
                  style={[styles.dayHeaderMonth, { color: colors.foreground }]}
                >
                  {formatZoned(selectedDate, 'MMMM yyyy')}
                </Text>
                <Text style={[styles.dayHeaderWeekday, { color: accentColor }]}>
                  {formatZoned(selectedDate, 'EEEE')}
                </Text>
              </View>
            </View>
          )}
          ...
          >
            <View style={{ flexDirection: 'row', height: '100%' }}>
              ...
              <View
                style={{
                  width: Dimensions.get('window').width - DAY_VIEW_OFFSET,
                }}
              >
                <View
                  style={{
                ...
                >
                  ...
                  {daysToRender.map((day, dayIndex) => {
                    ...
                    const processedEvents = dayEvents.map(event => {
                      ...
                      const endMins =
                        startMins !== null
                          ...
                          : undefined;
                      let durationMins =
                        startMins !== null && endMins !== undefined
                          ? endMins - startMins
                          : undefined;
                      ...
                      const height =
                        durationMins !== undefined
                          ? Math.max(25, (durationMins / 60) * HOUR_HEIGHT)
                          : undefined;
                      const bottom =
                    ...
                    const alldayItems = processedEvents.filter(
                      e => e.top === undefined || e.time === 'All Day',
                    );
                    const timedEvents = processedEvents.filter(
                      e => e.top !== undefined && e.time !== 'All Day',
                    ) as any[];
                    ...
                    timedEvents.forEach(event => {
                      if (currentCluster.length === 0) {
                        ...
                        if (event.top < clusterEnd - 0.1) {
                          // 0.1 buffer for floating point
                          currentCluster.push(event);
                    ...
                    return (
                      <React.Fragment key={dateStr}>
                        ...
                        {timedEvents.map(event => {
                          const eventWidth =
                            dayColumnWidthPercent / event.totalCols;
                          const eventLeft =
                            dayIndex * dayColumnWidthPercent +
                            event.colIndex * eventWidth;
                          ...
                          return (
                            <DraggableEvent
                              ...
                              isOwner={
                                activeMember?.id === event.memberId ||
                              ...
                              onUpdate={(id, updates) => {
                                if (event.type === 'task') {
                                  ...
                                  if (updates.date)
                                    taskUpdates.dateString = updates.date;
                                  if (updates.time)
                                    taskUpdates.dueDisplay = updates.time;
                                  updateTask(id, taskUpdates);
                                } else {
                                  const eventUpdates: any = {};
                                  if (updates.date)
                                    eventUpdates.dateString = updates.date;
                                  if (updates.time)
                                    eventUpdates.time = updates.time;
                  ...
                  {(activeView === 'Day' || activeView === 'Week') &&
                    isSameDay(selectedDate, zonedNow) && (
                      <View
                        style={{
                          position: 'absolute',
                          top: currentTimeTop,
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        >
          ...
          >
            ...
            >
              <AppIcon
                name="users"
                size={14}
                color={filterMember === null ? '#fff' : colors.foreground}
              />
              <Text
                style={[
                  styles.filterText,
            ...
            {members.map((member: any) => (
              ...
              >
                <MemberIcon
                  ...
                  size={16}
                  color={
                    filterMember === member.id ? '#fff' : colors.foreground
                  }
                />
                <Text
                  style={[
          ...
          {(activeView === 'Week' || activeView === 'Day') &&
            renderTimeline(activeView === 'Week' ? weekDays : [selectedDate])}
          ...
          <View style={styles.section}>
            ...
            {upcomingItems.slice(0, 3).map(event => {
              const displayDate = safeFormatInTimeZone(
                event.nextDate,
                timeZone,
                'MMM d',
              );
              const displayTime =
                event.time && event.time !== 'All Day' ? event.time : 'All Day';
              return (
                <Pressable

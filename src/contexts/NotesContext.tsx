...
export const NotesProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    const subscription = collection
      ...
      .subscribe({
        ...
        error: error =>
          console.error('Notes folder subscription failed', error),
      });
  ...
  useEffect(() => {
    ...
    const subscription = collection
      ...
      .subscribe({
        next: records => {
          console.log(
            `📥 NotesContext: Received ${records.length} notes update`,
          );
            ...
            .map(record => {
              const safeUpdatedAt =
                typeof record.updatedAt === 'number'
                  ? record.updatedAt
                  : Date.now();
              const safeBlocks = Array.isArray(record.blocks)
                ? record.blocks
                : [];
              return {
                id: record.id,
  ...
  const computePreview = (noteData?: Partial<Note>) => {
    ...
    const contentBlocks = blocks.filter(
      block => block.type !== 'divider' && block.content?.trim().length > 0,
    );
  ...
  const addNote = useCallback(
    async (folderId: string, noteData?: Partial<Note>) => {
  ...
  const updateNote = useCallback(
    async (noteId: string, updates: Partial<Note>) => {
      ...
      try {
        ...
        const wasPinned = Boolean(previousNote?.isStarred);
        await getDatabase().write(async () => {
        ...
        syncAfterWrite();
        if (updates.isStarred && !wasPinned) {
        }

...
import { parseLocalDateTime } from './SafeDateUtils';
...
const getEventTimeZone = (
  _event: CalendarEventWithMeta,
  _fallbackZone: string,
): string => {
  return '';
};
...
const getEventEndDate = (
  ...
  const endTimeCandidate =
    event.endTime && event.endTime !== 'All Day'
      ? event.endTime
...
const getRecurrenceEnd = (
  event: CalendarEventWithMeta,
  _eventTimeZone: string,
): Date | null => {
  if (event.__cachedRecurrenceEnd !== undefined)
    return event.__cachedRecurrenceEnd;
...
export const getEventsForDate = (
  ...
  const targetDateStr = `${baseTargetDate.getFullYear()}-${String(
    baseTargetDate.getMonth() + 1,
  ).padStart(2, '0')}-${String(baseTargetDate.getDate()).padStart(2, '0')}`;
  ...
  events.forEach(event => {
    ...
    if (event.isRecurring && event.recurrenceRule) {
      ...
      if (
        recurrenceEndZoned &&
        isAfter(targetDate, startOfDay(recurrenceEndZoned))
      )
        return;
      ...
      switch (event.recurrenceRule) {
        ...
        case 'biweekly':
          ...
          const diffDays = differenceInDays(
            targetDate,
            startOfDay(eventStartZoned),
          );
          const diffWeeks = Math.floor(diffDays / 7);
          isMatch =
            diffDays % 14 === 0 ||
            (isSameDayOfWeek(targetDate, eventStartDate) &&
              diffWeeks % 2 === 0);
          // Simple biweekly: needs to be same day of week AND even number of weeks apart
          isMatch =
            isSameDayOfWeek(targetDate, startOfDay(eventStartZoned)) &&
            Math.floor(
              differenceInDays(targetDate, startOfDay(eventStartZoned)) / 7,
            ) %
              2 ===
              0;
          break;
        case 'monthly':
          isMatch =
            targetDate.getDate() === startOfDay(eventStartZoned).getDate();
          break;
        case 'yearly':
          const startDay = startOfDay(eventStartZoned);
          isMatch =
            targetDate.getMonth() === startDay.getMonth() &&
            targetDate.getDate() === startDay.getDate();
          break;
        case 'weekday':
